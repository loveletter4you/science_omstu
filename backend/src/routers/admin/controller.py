...
async def controller_fill_authors(file: UploadFile, user: SchemeUser, db: Session):
    is_admin = await service_admin_check(user, db)
    if not is_admin:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Доступ запрещен")
...
async def controller_fill_white_list(file: UploadFile, user: SchemeUser, db: Session):
    if not service_admin_check(user, db):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Доступ запрещен")
    message = await service_white_list_fill(file, db)
    return message
